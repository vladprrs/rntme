...
- `docs/history/specs/active-rationale/` — per-feature designs (rationale embedded per spec)
...
5. The three follow-up specs from §6.2 are listed in `docs/history/specs/active-rationale/` planning notes (or as TODO comments in the relevant files) so they are not lost.
