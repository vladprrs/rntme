...
> **⚠️ SUPERSEDED (2026-04-18) — `backing: "derived"` rejection.** This plan and the surrounding /done execution log describe MVP `@rntme/qsm` as rejecting `backing: "derived"` with `QSM_BACKING_DERIVED_NOT_SUPPORTED` (see also §"Validate cross-ref" rule 6 and the cross-ref test "rejects backing: 'derived' (tier 2 not supported)" further down). The shipped runtime now **supports** derived projections via the hybrid idempotency design — see `docs/superpowers/specs/done/2026-04-18-d5-consumer-idempotency-hybrid-design.md` and the executed plan `docs/superpowers/plans/done/2026-04-18-d5-consumer-idempotency-hybrid.md`. Treat the "derived rejected" sections below as historical context only; do not reintroduce that constraint.
