...
**Related spec:** `docs/history/specs/historical/2026-04-14-mutations-design.md` §6 (QSM runtime-measurement), §7.7 (package layout).
...
> **⚠️ SUPERSEDED (2026-04-18) — `backing: "derived"` rejection.** This plan and the surrounding /done execution log describe MVP `@rntme/qsm` as rejecting `backing: "derived"` with `QSM_BACKING_DERIVED_NOT_SUPPORTED` (see also §"Validate cross-ref" rule 6 and the cross-ref test "rejects backing: 'derived' (tier 2 not supported)" further down). The shipped runtime now **supports** derived projections via the hybrid idempotency design — see `docs/history/specs/historical/2026-04-18-d5-consumer-idempotency-hybrid-design.md` and the executed plan `docs/history/plans/historical/2026-04-18-d5-consumer-idempotency-hybrid.md`. Treat the "derived rejected" sections below as historical context only; do not reintroduce that constraint.
...
Plan complete and saved to `docs/history/plans/historical/2026-04-14-rntme-qsm.md`. Two execution options:
