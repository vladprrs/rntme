...
export const provisioner: ProvisionerContract<MyConfig> = {
  async provision(input) {
    // input.publicConfig, input.targetSecrets, input.priorOutputs?,
    // input.serviceArtifacts?, input.log, input.signal
...
`ProvisionerInput` fields:

- `publicConfig` - module public config after deploy target var substitution.
- `targetSecrets` - resolved secret values for the target.
- `priorOutputs?` - previous provisioner outputs when reconciling.
- `serviceArtifacts?` - optional per-service validated artifacts keyed by service slug. Vendor provisioners that need service-level config (for example storage-s3 reading `storage.json` lifecycle rules) cast entries to their own branded validated type. Provisioners that do not need service artifacts ignore this field.
- `log` - structured provisioner logger supplied by the deploy executor.
- `signal` - abort signal honoring the module provisioner timeout.
...
Run package gates from `packages/contracts/provisioner/v1` with `bun test`,
`bun run typecheck`, `bun run build`, and `bun run lint`.

## Specs

- `docs/history/specs/active-rationale/2026-05-06-storage-s3-module-design.md` — `serviceArtifacts` consumer rationale for storage lifecycle provisioning.
