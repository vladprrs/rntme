...
function fieldTypeToProto(type: FieldType): { type: string; repeated: boolean } {
  switch (type.kind) {
    ...
    default: {
      ...
      void _exhaustive;
      throw new Error(`unsupported FieldType kind: ${(type as { kind: string }).kind}`);
    }
  }
}
...
export function shapeToProtoMessage(name: string, shape: ResolvedShape): string {
  ...
  for (const [fieldName, field] of Object.entries(shape.fields)) {
    ...
    // so a nullable array is emitted as a plain `repeated` field (an empty list
    // serves as the absent value).
