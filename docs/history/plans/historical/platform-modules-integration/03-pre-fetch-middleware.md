...
**Tech Stack:** Node 20, TypeScript strict, ESM, Vitest, pnpm. Reuses existing `@grpc/grpc-js@^1.10` + `protobufjs@^7.2` from plan 2. Adds `pino@^9` (structured logs) and reuses `prom-client@^15` already present in `@rntme/runtime`. Spec: `docs/history/specs/historical/2026-04-19-platform-modules-integration-design.md` §6.1, §7, §12.2.
