...
Module operations may return values; components that need those results should
use `useOperation`, while command-style buttons can keep using
`useModuleAction`.
...
React hook tests import `test/setup-dom.ts` to install a `happy-dom` window for
`bun:test`.
...
export async function boot(ctx: ModuleBootContext): Promise<void> {
  ...
  ctx.registerOperation('loadProfile', async () => {
    const res = await ctx.transport.fetch(new Request('/api/profile'));
    return res.json();
  });
...
export function UserBadge() {
  ...
  const loadProfile = useOperation('identity', 'loadProfile');
...
| `createOperationRegistry`, `OperationRegistry`, `OperationHandler` | Register and invoke module operations such as `login` / `logout` / data-returning storage calls. |
| `createLifecycleBus`, `LifecycleBus`, `LifecycleEvents` | In-process lifecycle event bus for host/module coordination. |
| `useTransport`, `useStateStore`, `useOperationRegistry`, `useOperation`, `useModuleAction` | React hooks for module components. |
...
- **Transport middleware is ordered.** Middleware runs in registration order,
  ...
  depend on authenticated fetches. Boot code that registers data-returning
  operations should call `ctx.transport.fetch(...)` so those operations use the
  same host transport chain.
...
Run package gates from `packages/contracts/client-runtime/v1` with `bun test`,
`bun run typecheck`, `bun run build`, and `bun run lint`.

## Specs

- [`../../../../docs/history/specs/historical/2026-05-04-platform-contracts-extraction-design.md`](/docs/history/specs/historical/2026-05-04-platform-contracts-extraction-design.md) — platform contracts extraction, including the `client-runtime/v1` split from `@rntme/ui-runtime`.
- [`../../../../docs/history/specs/active-rationale/2026-04-29-ui-module-contributions-design.md`](/docs/history/specs/active-rationale/2026-04-29-ui-module-contributions-design.md) — UI module contribution model and generated virtual entry.
