...
**Related:**
- `docs/history/specs/active-rationale/2026-04-26-modules-monorepo-structure-design.md` — defines `packages/contracts/<category>/v<n>/` layout, capability-based UNION conformance, governance rule for canonical growth (≥2 vendors OR archetypal), `module.json` schema. This spec produces the first concrete category-contract under that umbrella.
- `docs/history/specs/historical/2026-04-19-platform-modules-integration-design.md` — module pattern: wrapper around vendor SDK, gRPC surface, webhook receiver, no choreography. This spec's contract is implemented by Identity wrappers, never by gateways.
- `docs/history/specs/historical/2026-04-17-cloudevents-envelope-design.md` — CloudEvents 1.0 envelope; this spec defines the `data` payloads and `type` short-names for the Identity category.
...
- Implementation plans for this spec — `docs/history/plans/historical/identity-canonical-contract/`
...
This spec decomposes into **2 implementation plans** in `docs/history/plans/historical/identity-canonical-contract/`:
...
- `docs/history/specs/historical/2026-04-17-cloudevents-envelope-design.md` — envelope and `type` namespacing convention.
