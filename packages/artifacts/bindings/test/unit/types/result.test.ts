...
describe('result', () => {
  ...
  it('ERROR_CODES contains every documented code', () => {
    const expected: Array<keyof typeof ERROR_CODES> = [
      ...
      'BINDINGS_QUERY_ON_COMMAND_GRAPH',
      'BINDINGS_COMMAND_METHOD_NOT_POST',
  ...
  it('exposes command-kind error codes', () => {
    expect(ERROR_CODES.BINDINGS_COMMAND_ON_NON_COMMAND_GRAPH).toBe(
      'BINDINGS_COMMAND_ON_NON_COMMAND_GRAPH',
    );
    ...
    expect(ERROR_CODES.BINDINGS_COMMAND_QUERY_PARAM_FORBIDDEN).toBe(
      'BINDINGS_COMMAND_QUERY_PARAM_FORBIDDEN',
    );
  });
});
