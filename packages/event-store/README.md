...
- **`serviceName` is load-bearing and immutable per DB.** It flows into CE `source`, `type`, `dataSchema`, and the Kafka topic. `SqliteEventStore` records it in `event_store_metadata` on first initialization and rejects later opens with a different name via `EVENT_STORE_SERVICE_NAME_MISMATCH`.
