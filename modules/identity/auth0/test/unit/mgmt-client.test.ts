...
describe('Auth0 mgmt-client', () => {
  it('fetches an access token before the first call', async () => {
    ...
    const c = createMgmtClient({ tenantDomain: 'x.us.auth0.com', mgmtClientId: 'a', mgmtClientSecret: 'b', fetch: fetcher as typeof globalThis.fetch });
  ...
  it('retries 429 up to 3 times then fails', async () => {
    ...
    const c = createMgmtClient({ tenantDomain: 'x.us.auth0.com', mgmtClientId: 'a', mgmtClientSecret: 'b', fetch: fetcher as typeof globalThis.fetch, retryDelayMs: 1 });
