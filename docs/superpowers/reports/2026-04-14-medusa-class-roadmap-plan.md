...
**Хаб + тематические доки** в `docs/gaps/` (каталог уже существует и пуст — для этого и зарезервирован) + отдельный demo spec в `docs/superpowers/specs/` (соблюдение существующего naming convention `YYYY-MM-DD-<topic>-design.md`).
...
| 7 | `docs/superpowers/specs/2026-04-14-commerce-demo-design.md` | Spec на `demo/commerce-api`: минимальный домен (Product, Variant, Cart, LineItem, Order), 1–2 workflow (checkout), и явная карта "какие гэпы из roadmap блокируют demo сейчас". |
...
- `ls docs/gaps/` показывает 6 файлов; `ls docs/superpowers/specs/` содержит новый commerce-demo spec.
...
- `/home/coder/project/docs/superpowers/specs/2026-04-14-{bindings,bindings-http,mutations}-design.md` — текущие design notes
