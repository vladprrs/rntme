...
- Depends on: [`@rntme/pdm`](/docs/current/owners/packages/artifacts/pdm.md) for cross-ref validation (entity / field / state-machine / PDM-relation lookups via `PdmResolver`); [`zod`](https://zod.dev) for structural parsing; [`@rntme/pdm`](/docs/current/owners/packages/artifacts/pdm.md) `EventTypeSpec[]` for handler derivation.
- Consumed by: [`@rntme/projection-consumer`](/docs/current/owners/packages/runtime/projection-consumer.md) (reads `generateProjectionDdl` for table bootstrap and `deriveProjectionHandler` for per-event handlers); [`@rntme/graph-ir-compiler`](/docs/current/owners/packages/artifacts/graph-ir-compiler.md) (resolves projection table names and read-side relation graph via `QsmResolver`); [`@rntme/runtime`](/docs/current/owners/packages/runtime/runtime.md) (orchestrates parse / validate / DDL apply at boot).
...
- QSM relations are B2 cross-validated against PDM: `to`, `localKey`, `foreignKey`, `cardinality` must exactly equal the corresponding `PDM.entity.relations[name]`. PDM is canon; if QSM disagrees, the cross-ref layer fails. Source: spec [`docs/history/specs/historical/2026-04-16-qsm-relations-migration-design.md`](/docs/history/specs/historical/2026-04-16-qsm-relations-migration-design.md) §2.
...
- **Entity-mirror table-name resolution.** `validate/cross-ref.ts` normalizes absent entity-mirror `proj.table` values to the PDM `entity.table`; `derive/ddl.ts`, `derive/handler.ts`, and `createQsmResolver` then read the same validated table name. An older fallback to `defaultTableName(projName)` (e.g. `projection_noteview`) created a table the queries never read and surfaced as `SQLITE_ERROR: no such table` at the first SELECT — root cause of PR4 in [`docs/history/plans/historical/2026-05-04-notes-500-fix-pr4.md`](/docs/history/plans/historical/2026-05-04-notes-500-fix-pr4.md). Explicit `proj.table` remains supported; the structural duplicate-table check still uses `defaultTableName` as a name-only stand-in before PDM context is available.
- **SQLite DDL integration coverage.** `test/integration/ddl-bootstrap.test.ts`
  ...
  in-memory SQLite database through the Bun-backed `@rntme/sqlite` port, and
  verifies table/index/resolver alignment for explicit tables, omitted-table
  fallback, idempotency columns, and composite keys. This is test-only coverage;
  applying DDL in production remains owned by `@rntme/projection-consumer`.
...
- "Verify generated DDL against real SQLite" → run `bun test` from `packages/artifacts/qsm`; `test/integration/ddl-bootstrap.test.ts` boots emitted projection tables through `@rntme/sqlite`, checks indexes/idempotency columns, and asserts resolver table names match the physical tables.
...
- [`../../docs/history/specs/historical/2026-04-23-project-first-blueprint-design.md`](/docs/history/specs/historical/2026-04-23-project-first-blueprint-design.md) — historical umbrella design rationale for the project-first pivot: keeps `QSM` service-level, splits it into multiple files, and expands its allowed input scope to foreign-service events inside one project blueprint.
- [`../../docs/history/specs/historical/2026-04-14-mutations-design.md`](/docs/history/specs/historical/2026-04-14-mutations-design.md) — §6 entity-mirror projection contract: backing semantics, key/grain rules, generated columns, idempotency triple.
- [`../../docs/history/specs/historical/2026-04-16-qsm-relations-migration-design.md`](/docs/history/specs/historical/2026-04-16-qsm-relations-migration-design.md) — read-side relation graph migration from PDM to QSM: schema, B2 cross-validation rules, single-hop / fan-out gates, error codes.
