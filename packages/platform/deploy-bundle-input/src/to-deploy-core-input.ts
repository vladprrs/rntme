...
function platformServicePersistence(
  projectName: string,
  serviceSlug: string,
): { persistence?: { mode: 'persistent'; volumeName?: string; eventStorePath: string; qsmPath: string } } {
  if (projectName !== 'rntme-platform') return {};
  if (serviceSlug === 'tokens') {
    return {
      persistence: {
        mode: 'persistent',
  ...
  if (!['projects', 'deployments'].includes(serviceSlug)) return {};
  return {
    persistence: {
      mode: 'persistent',
      volumeName: 'rntme-platform-control-data',
      eventStorePath: `/srv/data/${serviceSlug}-events.sqlite`,
      qsmPath: '/srv/data/qsm.sqlite',
    },
  };
}
