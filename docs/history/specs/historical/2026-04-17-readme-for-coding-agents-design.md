...
- Modifying any spec in `docs/history/specs/active-rationale/` (existing specs are referenced, not rewritten).
...
**Sources of truth:**
- Domain rules: `docs/history/specs/active-rationale/*.md` and `graph_ir_rc_7.md` (referenced, never duplicated).
...
- Specs in `docs/history/specs/active-rationale/` are source of truth. Code that
...
- `docs/history/specs/active-rationale/` — authoritative design specs
- `docs/history/plans/historical/` — per-package implementation plans
- `docs/history/reports/` — gap analyses (spec vs implementation)
...
4. **All spec links resolve.** Each `docs/history/specs/active-rationale/<…>` reference grep-checked against the filesystem.
...
- Specs in `docs/history/specs/active-rationale/` are not edited.
