...
- **Entity-mirror table-name resolution.** `validate/cross-ref.ts` normalizes absent entity-mirror `proj.table` values to the PDM `entity.table`; `derive/ddl.ts`, `derive/handler.ts`, and `createQsmResolver` then read the same validated table name. An older fallback to `defaultTableName(projName)` (e.g. `projection_noteview`) created a table the queries never read and surfaced as `SQLITE_ERROR: no such table` at the first SELECT — root cause of PR4 in [`docs/superpowers/plans/done/2026-05-04-notes-500-fix-pr4.md`](/docs/superpowers/plans/done/2026-05-04-notes-500-fix-pr4.md). Explicit `proj.table` remains supported; the structural duplicate-table check still uses `defaultTableName` as a name-only stand-in before PDM context is available.
