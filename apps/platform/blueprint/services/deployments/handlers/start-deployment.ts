...
function createRuntimeResolveProvisioner(): ResolveProvisioner {
...
function appendRuntimeDeploymentLog(ctx: RuntimeCtx, deploymentId: string, line: SanitizedLogLine): void {
  ...
  `).run(
    String(nextDeploymentLogId(ctx, deploymentId)),
    deploymentId,
    line.level,
    line.step,
    line.message,
...
function nextDeploymentLogId(ctx: RuntimeCtx, deploymentId: string): number {
  const row = ctx.qsmDb.prepare<[string], { readonly id: number | null }>(`
