...
- `test/round-trip.test.ts` — bun:test encode/decode coverage.
...
- `bun run proto:gen` — regenerate `src/proto.gen.*` (do not edit generated files by hand).
- `bun run build` / `test` / `lint` / `typecheck` — package gates.

## Specs

- `docs/history/specs/historical/2026-04-26-identity-canonical-contract-design.md` §5 (`common.proto`).
