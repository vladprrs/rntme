...
- **Pre-fetch runs before query and command execution.** `runPreSteps` resolves `system` idempotency keys and `module-rpc` calls, then exposes results under graph input `pre` for `$pre.<bindAs>.<path>` expressions. Query bindings with `pre[]` require `externalAdapterClient`, the same as command bindings.
- **Inactive identity sessions map to 401.** `IntrospectSession` pre-step results whose canonical `Session.status` is not active fail before graph execution with `BINDINGS_AUTH_SESSION_INACTIVE`. Pre-step logs redact `vendor_raw.claims`.
