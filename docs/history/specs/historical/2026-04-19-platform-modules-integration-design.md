...
**Related:**
- `docs/history/specs/historical/2026-04-19-platform-api-design.md` (control-plane; §2 оговаривает, что gRPC surface и Zeebe integration отложены — эта спека закрывает часть отложенного)
...
- `docs/history/specs/historical/2026-04-17-cloudevents-envelope-design.md` (envelope и event-schema референс для модулей)
...
- Implementation plans для этой спеки — `docs/history/plans/historical/platform-modules-integration/`
...
Эта спека декомпозируется на **5 implementation plan'ов**, каждый пишется отдельно после approval этой спеки. Все планы лежат в `docs/history/plans/historical/platform-modules-integration/` (отдельная подпапка, не общий `plans/`).
