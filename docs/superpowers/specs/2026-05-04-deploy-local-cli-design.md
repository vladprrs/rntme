...
**Related:**
- `docs/superpowers/specs/done/2026-04-24-project-deployment-pipeline-design.md` — `provision → plan → render → apply → verify` pipeline; this spec adds a new render+apply pair (`@rntme/deploy-local`) following that pattern.
- `docs/superpowers/specs/done/2026-05-01-provisioned-event-bus-design.md` — Redpanda compose generator (`renderRedpandaCompose`); this spec extracts it to a target-neutral helper so both Dokploy and Local adapters share it.
- `docs/superpowers/specs/done/2026-05-03-module-provisioner-contract-design.md` — module provisioner contract; this spec defines local-mode behavior (skipped by default, opt-in via `--provision`).
- `docs/superpowers/specs/2026-05-04-notes-demo-fresh-tenant-deployable-design.md` — fresh-tenant deploy via Dokploy. This spec is the local-mode counterpart; the same notes-blueprint should run via `rntme local up` without any Auth0 tenant at all.
