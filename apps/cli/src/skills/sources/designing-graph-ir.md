...
| Dot-navigation across service boundaries (e.g. `issue.userFromOtherService.email`) | Cross-service joins are forbidden — they couple services at the read layer. Cross-service reads must go through Operaton BPMN orchestration (`rntme_orchestration_only`). Only navigate relations declared in the current service's `qsm.json`. |
