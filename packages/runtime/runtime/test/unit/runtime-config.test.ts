...
describe('validateRuntimeConfig', () => {
  ...
  it('accepts valid custom runtime plugin shapes', () => {
    const config = {
      ...
      onReady: () => undefined,
      skipSeed: false,
      commandExecutor: { execute: async () => ({ ok: false, error: { code: 'COMMAND_NOT_FOUND', message: 'x' } }) },
      queryExecutor: { execute: async () => ({ ok: false, error: { code: 'QUERY_NOT_FOUND', message: 'x' } }) },
