...
describeOrSkip('rntme platform up --target <file>', () => {
  ...
  it('provisions platform services successfully', () => {
    if (live === null) throw new Error('RNTME_DOKPLOY_E2E env not configured');
    const tmp = mkdtempSync(join(tmpdir(), 'cli-platform-up-e2e-'));
    ...
    const result = spawnSync(process.execPath, [cliPath, 'platform', 'up', '--target', targetPath], {
      encoding: 'utf8',
