...
| Q9 | Existing references to `vision.md` in active specs and worktrees | **Leave.** Active specs in `docs/superpowers/specs/*.md` and `done/*.md` may continue to reference the deleted file as historical context. The reader follows from the spec date and infers. |
...
| `docs/superpowers/specs/*.md` (active and `done/`) | **Out of scope.** Historical references to `vision.md` are left intact. |
...
- No retroactive edits to historical specs in `docs/superpowers/specs/done/`.
- No edits to active specs in `docs/superpowers/specs/*.md` that mention `vision.md`. They keep their references as historical context; the reader infers from spec dates.
