...
```bash
rg -n "pre\\[\\]|kind: \\\"query\\\"|kind: \\\"command\\\"|row<CommandResult>|commands\\.handlersModule|services/.*/commands/handlers\\.mjs|inferRole\\(\\)" AGENTS.md README.md packages apps demo docs/history/specs/historical/2026-05-06-graph-ir-effect-operations-design.md
```

Expected: only historical specs/plans under `docs/history/specs/historical/` and `docs/history/plans/historical/` may still mention old terms. Current READMEs, AGENTS, demos, and CLI skill sources must not present old terms as current guidance.
