...
- Depends on: `@rntme/sqlite` (runtime SQLite port over `bun:sqlite`).
...
src/
  ...
  store/
    ...
    sqlite.ts                (entry) SqliteEventStore class + mapSqliteError; opens `@rntme/sqlite`, enforces one live writer per DB file in-process, pins DB serviceName metadata, runs DDL, asserts D9 schema, implements append/read/cursor/delivery-tracking/raw.
...
| `SqliteEventStore#rawDb()`                                                                                         | Test/advanced escape hatch — returns the `@rntme/sqlite` `SqliteDatabase` port.                                                                                                                                                                                                                                                                                                                                                                                                       |
